  ...
  const confirmPayment = async () => {
    ...
    try {
      const response = await walletAPI.payerFacture({
        service_type: 'ZIZ',
        service_id: 1, // ID du service ZIZ
    ...
    <>
      <Card className="shadow-sm border-0">
        <Card.Body className="p-4">
          <div className="text-center mb-4">
            <div className="icon-wrapper bg-warning rounded-circle mx-auto mb-3" 
                 style={{ width: '80px', height: '80px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <i className="fas fa-bolt fa-2x text-white"></i>
            </div>
            <h4 className="fw-bold">Paiement Facture ZIZ</h4>
            <p className="text-muted">
              Société Nationale d'Électricité du Tchad
            </p>
          </div>
              ...
              <Col md={6}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    placeholder="Votre numéro de client ZIZ"
                    required
                  />
                </Form.Group>
              </Col>
      ...
      <Modal show={showConfirmation} onHide={() => setShowConfirmation(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Confirmer le paiement ZIZ</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>Êtes-vous sûr de vouloir effectuer ce paiement?</p>
          <div className="bg-light p-3 rounded">
            <strong>Service:</strong> ZIZ - Électricité<br/>
        ...
        <Modal.Footer>
          ...
          <Button variant="warning" onClick={confirmPayment} disabled={loading}>
